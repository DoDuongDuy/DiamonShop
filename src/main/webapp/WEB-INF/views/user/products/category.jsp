...
<body>
	...
	<div class="well well-small">
		...
		<c:if test="${ ProductsPanigate.size() > 0 }">
			<div class="row-fluid">
				<ul class="thumbnails">

					<c:forEach var="item" items="${ ProductsPanigate }"
						varStatus="loop">
						<li class="span4">
							<div class="thumbnail">
								...
								<div class="caption cntr">
									...
									<p>
										<strong><fmt:formatNumber type="number"
												groupingUsed="true" value="${ item.price }" /> ₫</span></strong>
									</p>
									<h4>
								...
								test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == ProductsPanigate.size() }">
				</ul>
			</div>
			<c:if test="${ (loop.index + 1) < ProductsPanigate.size() }">
				<div class="row-fluid">
					<ul class="thumbnails">
			</c:if>
		</c:if>
	...
	<div class="pagination">
		...
		<c:forEach var="item" begin="1" end="${paginatecInfo.totalPage }" varStatus="loop">
			<c:if test="${ (loop.index ) == paginatecInfo.currentPage }">
